...
void get(char* path, char* response_buffer) {
...
void post(char* path, char* response_buffer, char* message) {
    esp_http_client_config_t config = {
        .host = HASS_URL,
        .port = HASS_PORT,
        .path = path,
    ...
    ESP_LOGI(TAG, "HTTP POST at %s Yielded: %s", path, response_buffer);
    esp_http_client_cleanup(client);
}


static void http_test_connection(void *pvParameters) {
...
void app_hass_init(void)
{
    ...
    xTaskCreate(&http_test_connection, "http_test_task", 8192, NULL, 5, NULL);
}
